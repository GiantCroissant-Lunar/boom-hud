// <auto-generated>
// Generated by BoomHud.Gen.Unity from CharPortrait
// </auto-generated>

#nullable enable

using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.Globalization;
using System.Linq;
using UnityEngine;
using UnityEngine.TextCore.Text;
using UnityEngine.UIElements;

namespace Generated.Hud
{
public sealed class CharPortraitView
{
    private readonly ICharPortraitViewModel? _initialViewModel = null;
    private ICharPortraitViewModel? _viewModel;

    public VisualElement Root { get; }
    public VisualElement Face { get; }
    public Label ClassIcon { get; }
    public Label Name { get; }
    public VisualElement Hp { get; }
    public VisualElement HpFill { get; }
    public VisualElement Mp { get; }
    public VisualElement MpFill { get; }
    public VisualElement Stats { get; }
    public Label Stat1 { get; }
    public Label Stat2 { get; }
    public VisualElement ActionGrid { get; }
    public VisualElement Atk { get; }
    public Label QEpO3 { get; }
    public VisualElement Mag { get; }
    public Label AIphN { get; }
    public VisualElement Def { get; }
    public Label E4QKZ { get; }
    public VisualElement Item { get; }
    public Label DVzX7 { get; }

    public ICharPortraitViewModel? ViewModel
    {
        get => _viewModel;
        set
        {
            if (_viewModel != null)
            {
                _viewModel.PropertyChanged -= OnViewModelPropertyChanged;
            }

            _viewModel = value;

            if (_viewModel != null)
            {
                _viewModel.PropertyChanged += OnViewModelPropertyChanged;
            }

            Refresh();
        }
    }

    public CharPortraitView(VisualElement root)
    {
        Root = root as VisualElement ?? throw new ArgumentException("Expected root element type VisualElement.", nameof(root));
        Face = Root.Q<VisualElement>("Face") ?? throw new InvalidOperationException("Could not find generated element 'Face'.");
        ClassIcon = Root.Q<Label>("ClassIcon") ?? throw new InvalidOperationException("Could not find generated element 'ClassIcon'.");
        Name = Root.Q<Label>("Name") ?? throw new InvalidOperationException("Could not find generated element 'Name'.");
        Hp = Root.Q<VisualElement>("Hp") ?? throw new InvalidOperationException("Could not find generated element 'Hp'.");
        HpFill = Root.Q<VisualElement>("HpFill") ?? throw new InvalidOperationException("Could not find generated element 'HpFill'.");
        Mp = Root.Q<VisualElement>("Mp") ?? throw new InvalidOperationException("Could not find generated element 'Mp'.");
        MpFill = Root.Q<VisualElement>("MpFill") ?? throw new InvalidOperationException("Could not find generated element 'MpFill'.");
        Stats = Root.Q<VisualElement>("Stats") ?? throw new InvalidOperationException("Could not find generated element 'Stats'.");
        Stat1 = Root.Q<Label>("Stat1") ?? throw new InvalidOperationException("Could not find generated element 'Stat1'.");
        Stat2 = Root.Q<Label>("Stat2") ?? throw new InvalidOperationException("Could not find generated element 'Stat2'.");
        ActionGrid = Root.Q<VisualElement>("ActionGrid") ?? throw new InvalidOperationException("Could not find generated element 'ActionGrid'.");
        Atk = Root.Q<VisualElement>("Atk") ?? throw new InvalidOperationException("Could not find generated element 'Atk'.");
        QEpO3 = Root.Q<Label>("QEpO3") ?? throw new InvalidOperationException("Could not find generated element 'QEpO3'.");
        Mag = Root.Q<VisualElement>("Mag") ?? throw new InvalidOperationException("Could not find generated element 'Mag'.");
        AIphN = Root.Q<Label>("AIphN") ?? throw new InvalidOperationException("Could not find generated element 'AIphN'.");
        Def = Root.Q<VisualElement>("Def") ?? throw new InvalidOperationException("Could not find generated element 'Def'.");
        E4QKZ = Root.Q<Label>("E4QKZ") ?? throw new InvalidOperationException("Could not find generated element 'E4QKZ'.");
        Item = Root.Q<VisualElement>("Item") ?? throw new InvalidOperationException("Could not find generated element 'Item'.");
        DVzX7 = Root.Q<Label>("DVzX7") ?? throw new InvalidOperationException("Could not find generated element 'DVzX7'.");

        ApplyStaticValues();
        ViewModel = _initialViewModel;
    }

    public void Refresh()
    {
        if (_viewModel == null)
        {
            return;
        }

    }

    private void ApplyStaticValues()
    {
        Root.style.display = DisplayStyle.Flex;
        Root.SetEnabled(true);
        Root.style.color = ParseStyleColor("#000000", null);
        Root.style.backgroundColor = ParseStyleColor("#000000", null);
        Face.style.display = DisplayStyle.Flex;
        Face.SetEnabled(true);
        Face.style.color = ParseStyleColor("#000000", null);
        Face.style.backgroundColor = ParseStyleColor("#000000", null);
        Face.style.borderLeftWidth = 6f;
        Face.style.borderRightWidth = 6f;
        Face.style.borderTopWidth = 6f;
        Face.style.borderBottomWidth = 6f;
        Face.style.borderLeftColor = ParseStyleColor("#FFFFFF", null);
        Face.style.borderRightColor = ParseStyleColor("#FFFFFF", null);
        Face.style.borderTopColor = ParseStyleColor("#FFFFFF", null);
        Face.style.borderBottomColor = ParseStyleColor("#FFFFFF", null);
        ClassIcon.text = ResolveIconText("shield", "lucide", 32f);
        ApplyIconLabelStyle(ClassIcon, 32f, 32f);
        ClassIcon.style.display = DisplayStyle.Flex;
        ClassIcon.SetEnabled(true);
        ApplyFontFamily(ClassIcon, "lucide", 32f);
        ClassIcon.style.color = ParseStyleColor("#FFFFFF", null);
        Name.text = "Name";
        ApplyTextLabelStyle(Name);
        Name.style.display = DisplayStyle.Flex;
        Name.SetEnabled(true);
        ApplyFontFamily(Name, "Press Start 2P", 8f);
        Name.style.color = ParseStyleColor("#FFFFFF", null);
        Hp.style.display = DisplayStyle.Flex;
        Hp.SetEnabled(true);
        Hp.style.color = ParseStyleColor("#333333", null);
        Hp.style.backgroundColor = ParseStyleColor("#333333", null);
        HpFill.style.display = DisplayStyle.Flex;
        HpFill.SetEnabled(true);
        HpFill.style.color = ParseStyleColor("#CCCCCC", null);
        HpFill.style.backgroundColor = ParseStyleColor("#CCCCCC", null);
        Mp.style.display = DisplayStyle.Flex;
        Mp.SetEnabled(true);
        Mp.style.color = ParseStyleColor("#333333", null);
        Mp.style.backgroundColor = ParseStyleColor("#333333", null);
        MpFill.style.display = DisplayStyle.Flex;
        MpFill.SetEnabled(true);
        MpFill.style.color = ParseStyleColor("#777777", null);
        MpFill.style.backgroundColor = ParseStyleColor("#777777", null);
        Stats.style.display = DisplayStyle.Flex;
        Stats.SetEnabled(true);
        Stat1.text = "ATK 10";
        ApplyTextLabelStyle(Stat1);
        Stat1.style.display = DisplayStyle.Flex;
        Stat1.SetEnabled(true);
        ApplyFontFamily(Stat1, "Press Start 2P", 8f);
        Stat1.style.color = ParseStyleColor("#AAAAAA", null);
        Stat2.text = "DEF 8";
        ApplyTextLabelStyle(Stat2);
        Stat2.style.display = DisplayStyle.Flex;
        Stat2.SetEnabled(true);
        ApplyFontFamily(Stat2, "Press Start 2P", 8f);
        Stat2.style.color = ParseStyleColor("#AAAAAA", null);
        ActionGrid.style.display = DisplayStyle.Flex;
        ActionGrid.SetEnabled(true);
        Atk.style.display = DisplayStyle.Flex;
        Atk.SetEnabled(true);
        Atk.style.color = ParseStyleColor("#000000", null);
        Atk.style.backgroundColor = ParseStyleColor("#000000", null);
        Atk.style.borderLeftWidth = 2f;
        Atk.style.borderRightWidth = 2f;
        Atk.style.borderTopWidth = 2f;
        Atk.style.borderBottomWidth = 2f;
        Atk.style.borderLeftColor = ParseStyleColor("#FFFFFF", null);
        Atk.style.borderRightColor = ParseStyleColor("#FFFFFF", null);
        Atk.style.borderTopColor = ParseStyleColor("#FFFFFF", null);
        Atk.style.borderBottomColor = ParseStyleColor("#FFFFFF", null);
        QEpO3.text = ResolveIconText("swords", "lucide", 16f);
        ApplyIconLabelStyle(QEpO3, 16f, 16f);
        QEpO3.style.display = DisplayStyle.Flex;
        QEpO3.SetEnabled(true);
        ApplyFontFamily(QEpO3, "lucide", 16f);
        QEpO3.style.color = ParseStyleColor("#FFFFFF", null);
        Mag.style.display = DisplayStyle.Flex;
        Mag.SetEnabled(true);
        Mag.style.color = ParseStyleColor("#000000", null);
        Mag.style.backgroundColor = ParseStyleColor("#000000", null);
        Mag.style.borderLeftWidth = 2f;
        Mag.style.borderRightWidth = 2f;
        Mag.style.borderTopWidth = 2f;
        Mag.style.borderBottomWidth = 2f;
        Mag.style.borderLeftColor = ParseStyleColor("#FFFFFF", null);
        Mag.style.borderRightColor = ParseStyleColor("#FFFFFF", null);
        Mag.style.borderTopColor = ParseStyleColor("#FFFFFF", null);
        Mag.style.borderBottomColor = ParseStyleColor("#FFFFFF", null);
        AIphN.text = ResolveIconText("wand-sparkles", "lucide", 16f);
        ApplyIconLabelStyle(AIphN, 16f, 16f);
        AIphN.style.display = DisplayStyle.Flex;
        AIphN.SetEnabled(true);
        ApplyFontFamily(AIphN, "lucide", 16f);
        AIphN.style.color = ParseStyleColor("#FFFFFF", null);
        Def.style.display = DisplayStyle.Flex;
        Def.SetEnabled(true);
        Def.style.color = ParseStyleColor("#000000", null);
        Def.style.backgroundColor = ParseStyleColor("#000000", null);
        Def.style.borderLeftWidth = 2f;
        Def.style.borderRightWidth = 2f;
        Def.style.borderTopWidth = 2f;
        Def.style.borderBottomWidth = 2f;
        Def.style.borderLeftColor = ParseStyleColor("#FFFFFF", null);
        Def.style.borderRightColor = ParseStyleColor("#FFFFFF", null);
        Def.style.borderTopColor = ParseStyleColor("#FFFFFF", null);
        Def.style.borderBottomColor = ParseStyleColor("#FFFFFF", null);
        E4QKZ.text = ResolveIconText("shield", "lucide", 16f);
        ApplyIconLabelStyle(E4QKZ, 16f, 16f);
        E4QKZ.style.display = DisplayStyle.Flex;
        E4QKZ.SetEnabled(true);
        ApplyFontFamily(E4QKZ, "lucide", 16f);
        E4QKZ.style.color = ParseStyleColor("#FFFFFF", null);
        Item.style.display = DisplayStyle.Flex;
        Item.SetEnabled(true);
        Item.style.color = ParseStyleColor("#000000", null);
        Item.style.backgroundColor = ParseStyleColor("#000000", null);
        Item.style.borderLeftWidth = 2f;
        Item.style.borderRightWidth = 2f;
        Item.style.borderTopWidth = 2f;
        Item.style.borderBottomWidth = 2f;
        Item.style.borderLeftColor = ParseStyleColor("#FFFFFF", null);
        Item.style.borderRightColor = ParseStyleColor("#FFFFFF", null);
        Item.style.borderTopColor = ParseStyleColor("#FFFFFF", null);
        Item.style.borderBottomColor = ParseStyleColor("#FFFFFF", null);
        DVzX7.text = ResolveIconText("flask-conical", "lucide", 16f);
        ApplyIconLabelStyle(DVzX7, 16f, 16f);
        DVzX7.style.display = DisplayStyle.Flex;
        DVzX7.SetEnabled(true);
        ApplyFontFamily(DVzX7, "lucide", 16f);
        DVzX7.style.color = ParseStyleColor("#FFFFFF", null);
    }

    private void OnViewModelPropertyChanged(object? sender, PropertyChangedEventArgs e)
    {
        Refresh();
    }

    private static readonly Dictionary<string, FontDefinition> s_fontDefinitions = new(StringComparer.OrdinalIgnoreCase);
    private static readonly Dictionary<string, FontAsset> s_sdfFontAssets = new(StringComparer.OrdinalIgnoreCase);
    private static readonly Dictionary<string, string> s_fontResourcePaths = new(StringComparer.OrdinalIgnoreCase)
    {
        ["Press Start 2P"] = "BoomHudFonts/PressStart2P-Regular",
        ["lucide"] = "BoomHudFonts/lucide"
    };
    private static readonly Dictionary<string, string> s_lucideGlyphs = new(StringComparer.OrdinalIgnoreCase)
    {
        ["cross"] = "\uE1E5",
        ["flame"] = "\uE0D2",
        ["flask-conical"] = "\uE0D5",
        ["moon"] = "\uE11E",
        ["shield"] = "\uE158",
        ["sparkles"] = "\uE412",
        ["sword"] = "\uE2B3",
        ["swords"] = "\uE2B4",
        ["wand"] = "\uE246",
        ["wand-2"] = "\uE357",
        ["wand-sparkles"] = "\uE357"
    };

    private static string AsString(object? value) => value?.ToString() ?? string.Empty;

    private static float AsFloat(object? value)
    {
        return value switch
        {
            null => 0f,
            float floatValue => floatValue,
            double doubleValue => (float)doubleValue,
            int intValue => intValue,
            long longValue => longValue,
            decimal decimalValue => (float)decimalValue,
            string stringValue when float.TryParse(stringValue, NumberStyles.Float, CultureInfo.InvariantCulture, out var parsed) => parsed,
            _ => 0f
        };
    }

    private static bool AsBool(object? value)
    {
        return value switch
        {
            bool boolValue => boolValue,
            string stringValue when bool.TryParse(stringValue, out var parsed) => parsed,
            int intValue => intValue != 0,
            long longValue => longValue != 0,
            _ => false
        };
    }

    private static string? ResolveMappedStyleValue(object? value, string? fallbackValue, params string[] mappings)
    {
        var key = AsString(value);

        for (var index = 0; index + 1 < mappings.Length; index += 2)
        {
            if (string.Equals(mappings[index], key, StringComparison.OrdinalIgnoreCase))
            {
                return mappings[index + 1];
            }
        }

        return string.IsNullOrWhiteSpace(key) ? fallbackValue : key;
    }

    private static Color ParseStyleColor(string? value, string? fallbackValue)
    {
        if (!string.IsNullOrWhiteSpace(value) && ColorUtility.TryParseHtmlString(value, out var parsed))
        {
            return parsed;
        }

        if (!string.IsNullOrWhiteSpace(fallbackValue) && ColorUtility.TryParseHtmlString(fallbackValue, out var fallback))
        {
            return fallback;
        }

        return default;
    }

    private static void ApplyTextLabelStyle(Label label)
    {
        label.style.whiteSpace = WhiteSpace.NoWrap;
        label.style.flexShrink = 0;
        label.style.overflow = Overflow.Visible;
    }

    private static bool ApplyFontFamily(VisualElement element, string? familyName, float pointSize)
    {
        if (element == null || string.IsNullOrWhiteSpace(familyName))
        {
            return false;
        }

        if (!TryResolveFontDefinition(familyName, pointSize, out var fontDefinition))
        {
            return false;
        }

        element.style.unityFontDefinition = fontDefinition;
        return true;
    }

    private static bool TryResolveFontDefinition(string? familyName, float pointSize, out FontDefinition fontDefinition)
    {
        fontDefinition = default;

        if (string.IsNullOrWhiteSpace(familyName))
        {
            return false;
        }

        var normalizedFamily = familyName.Trim();
        var normalizedSize = Mathf.Max(1, Mathf.RoundToInt(pointSize));
        var cacheKey = $"{normalizedFamily}|{normalizedSize}";

        if (s_fontDefinitions.TryGetValue(cacheKey, out fontDefinition))
        {
            return true;
        }

        if (TryLoadSdfFontAsset(normalizedFamily, out var fontAsset))
        {
            fontDefinition = FontDefinition.FromSDFFont(fontAsset);
            s_fontDefinitions[cacheKey] = fontDefinition;
            return true;
        }

        if (!TryLoadFont(normalizedFamily, normalizedSize, out var font))
        {
            return false;
        }

        fontDefinition = FontDefinition.FromFont(font);
        s_fontDefinitions[cacheKey] = fontDefinition;
        return true;
    }

    private static bool TryLoadSdfFontAsset(string familyName, out FontAsset fontAsset)
    {
        fontAsset = null!;

        var normalizedFamily = familyName.Trim();
        if (s_sdfFontAssets.TryGetValue(normalizedFamily, out fontAsset))
        {
            return fontAsset != null;
        }

        fontAsset = LoadBundledSdfFontAsset(normalizedFamily)!;
        if (fontAsset == null)
        {
            return false;
        }

        s_sdfFontAssets[normalizedFamily] = fontAsset;
        return true;
    }

    private static FontAsset? LoadBundledSdfFontAsset(string familyName)
    {
        var normalizedFamily = familyName.Trim();

        if (s_fontResourcePaths.TryGetValue(normalizedFamily, out var resourcePath))
        {
            return Resources.Load<FontAsset>(resourcePath);
        }

        var compactName = normalizedFamily.Replace(" ", string.Empty, StringComparison.Ordinal).Replace("-", string.Empty, StringComparison.Ordinal);
        return Resources.Load<FontAsset>($"BoomHudFonts/{compactName}") ?? Resources.Load<FontAsset>($"BoomHudFonts/{normalizedFamily}");
    }

    private static bool TryLoadFont(string familyName, int pointSize, out Font font)
    {
        font = null!;

        var resourceFont = LoadBundledFont(familyName);
        if (resourceFont != null)
        {
            font = resourceFont;
            return true;
        }

        var fontCandidates = ExpandFontFamilyCandidates(familyName);
        foreach (var candidate in fontCandidates)
        {
            resourceFont = LoadBundledFont(candidate);
            if (resourceFont != null)
            {
                font = resourceFont;
                return true;
            }
        }

        try
        {
            var osFont = Font.CreateDynamicFontFromOSFont(fontCandidates, pointSize);
            if (osFont == null)
            {
                return false;
            }

            font = osFont;
            return true;
        }
        catch
        {
            return false;
        }
    }

    private static Font? LoadBundledFont(string familyName)
    {
        var normalizedFamily = familyName.Trim();

        if (s_fontResourcePaths.TryGetValue(normalizedFamily, out var resourcePath))
        {
            return Resources.Load<Font>(resourcePath);
        }

        var compactName = normalizedFamily.Replace(" ", string.Empty, StringComparison.Ordinal).Replace("-", string.Empty, StringComparison.Ordinal);
        return Resources.Load<Font>($"BoomHudFonts/{compactName}") ?? Resources.Load<Font>($"BoomHudFonts/{normalizedFamily}");
    }

    private static string[] ExpandFontFamilyCandidates(string familyName)
    {
        var rawCandidates = familyName.Split(',');
        var expanded = new List<string>();

        foreach (var rawCandidate in rawCandidates)
        {
            var candidate = rawCandidate.Trim().Trim('\'', '"');
            if (string.IsNullOrWhiteSpace(candidate))
            {
                continue;
            }

            switch (candidate.ToLowerInvariant())
            {
                case "monospace":
                    expanded.Add("Consolas");
                    expanded.Add("Cascadia Mono");
                    expanded.Add("Courier New");
                    expanded.Add("Lucida Console");
                    break;
                case "sans-serif":
                case "sans serif":
                    expanded.Add("Segoe UI");
                    expanded.Add("Arial");
                    break;
                case "serif":
                    expanded.Add("Georgia");
                    expanded.Add("Times New Roman");
                    break;
                default:
                    expanded.Add(candidate);
                    break;
            }
        }

        if (expanded.Count == 0)
        {
            expanded.Add(familyName.Trim());
        }

        return expanded.Distinct(StringComparer.OrdinalIgnoreCase).ToArray();
    }

    private static string ResolveIconText(string value, string? familyName, float pointSize)
    {
        if (!TryResolveFontDefinition(familyName, pointSize, out _))
        {
            return NormalizeIconText(value);
        }

        if (TryResolveIconGlyphText(value, familyName, out var glyphText))
        {
            return glyphText;
        }

        return string.Equals(familyName?.Trim(), "lucide", StringComparison.OrdinalIgnoreCase) ? NormalizeIconText(value) : value;
    }

    private static bool TryResolveIconGlyphText(string value, string? familyName, out string glyphText)
    {
        glyphText = string.Empty;

        if (string.IsNullOrWhiteSpace(familyName))
        {
            return false;
        }

        var normalizedFamily = familyName.Trim();
        if (string.Equals(normalizedFamily, "lucide", StringComparison.OrdinalIgnoreCase) && s_lucideGlyphs.TryGetValue(value, out glyphText))
        {
            return true;
        }

        return false;
    }

    private static string NormalizeIconText(string value)
    {
        return value switch
        {
            "sword" => "†",
            "swords" => "⚔",
            "sparkles" => "✦",
            "wand-sparkles" => "✦",
            "shield" => "⛨",
            "flask-conical" => "⚗",
            "flame" => "✹",
            "moon" => "☾",
            "cross" => "✚",
            _ => value
        };
    }

    private static void ApplyIconLabelStyle(Label label, float boxWidth, float boxHeight)
    {
        var iconSize = Mathf.Max(1f, Mathf.Min(boxWidth, boxHeight));
        label.style.unityTextAlign = TextAnchor.MiddleCenter;
        label.style.unityFontStyleAndWeight = FontStyle.Normal;
        label.style.whiteSpace = WhiteSpace.NoWrap;
        label.style.flexShrink = 0;
        label.style.alignItems = Align.Center;
        label.style.justifyContent = Justify.Center;
        label.style.overflow = Overflow.Visible;
        label.style.paddingLeft = 0f;
        label.style.paddingTop = 0f;
        label.style.paddingRight = 0f;
        label.style.paddingBottom = 0f;
        label.style.width = boxWidth;
        label.style.height = boxHeight;
        label.style.minWidth = boxWidth;
        label.style.minHeight = boxHeight;
        if (boxWidth >= 32f && boxHeight >= 32f)
        {
            label.style.marginTop = -1f;
        }
        label.style.fontSize = iconSize;
    }
}
}

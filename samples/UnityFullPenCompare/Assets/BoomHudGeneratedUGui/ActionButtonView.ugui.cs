...
public sealed class ActionButtonView
{
    ...
    public ActionButtonView(Transform? parent = null, IActionButtonViewModel? viewModel = null, IReadOnlyDictionary<string, IReadOnlyDictionary<string, object?>>? componentOverrides = null)
    {
        _componentOverrides = componentOverrides;
        Root = CreateRect("ActionButtonRoot", parent);
        ConfigureRect(Root, width: 28f, height: 28f, left: null, top: null, absolute: false);
        ApplyLayoutSizing(Root, ignoreLayout: false, preferredWidth: 28f, preferredHeight: 28f, flexibleWidth: null, flexibleHeight: null);
        ApplyContentSizeFit(Root, horizontal: false, vertical: false);
        ApplyVerticalLayout(Root, 0f, 0, 0, 0, 0, "center", childControlWidth: false, childControlHeight: false);
        ...
        Icon = CreateText("Icon", Root);
        ConfigureRect(RectOf(Icon), width: 16f, height: 16f, left: null, top: null, absolute: false);
        ApplyLayoutSizing(RectOf(Icon), ignoreLayout: false, preferredWidth: 16f, preferredHeight: 16f, flexibleWidth: null, flexibleHeight: null);
        ApplyContentSizeFit(RectOf(Icon), horizontal: false, vertical: false);
        ApplyStyle(Icon, fg: "#FFFFFF", bg: null, fontFamily: "lucide", fontSize: 16, borderColor: null, borderWidth: null, treatAsIcon: true);
        ApplyIconMetrics(Icon, boxWidth: 16f, boxHeight: 16f, baselineOffset: 0f, opticalCentering: true, sizeMode: "fit-box", explicitFontSize: 16f);
        ApplyTextMetrics(Icon, lineSpacing: null, wrapText: false);
        Icon.text = ResolveIconText("swords");
        Icon.gameObject.SetActive(true);
        ApplyInstanceOverrides();
        ViewModel = viewModel;
    }

    private ActionButtonView(RectTransform root, IActionButtonViewModel? viewModel, IReadOnlyDictionary<string, IReadOnlyDictionary<string, object?>>? componentOverrides)
    {
        _componentOverrides = componentOverrides;
        Root = root;
        Icon = RequireComponent<Text>(Root, "Icon");
        ApplyInstanceOverrides();
        ViewModel = viewModel;
    }

    public static ActionButtonView Bind(RectTransform root, IActionButtonViewModel? viewModel = null, IReadOnlyDictionary<string, IReadOnlyDictionary<string, object?>>? componentOverrides = null) => new(root, viewModel, componentOverrides);
    ...
    private void ApplyInstanceOverrides()
    {
        ...
        {
            Icon.text = ResolveIconText(AsString(componentOverrideValue0));
        }
    }

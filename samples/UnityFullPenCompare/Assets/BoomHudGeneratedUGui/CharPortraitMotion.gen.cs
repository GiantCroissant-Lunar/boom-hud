// <auto-generated>
// Generated by BoomHud.Gen.UGui from motion document CharPortraitMotion
// </auto-generated>
...
public static class CharPortraitMotion
{
    ...
    private static void ApplyOpacity(Component target, float value)
    {
    ...
    private static void ApplyWidth(Component target, float value)
    {
        ResolveRectTransform(target).SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, value);
    }

    private static void ApplyHeight(Component target, float value)
    {
        ResolveRectTransform(target).SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, value);
    }

    private static void ApplyVisibility(Component target, bool visible)
    {
        target.gameObject.SetActive(visible);
    }

    private static void ApplyTranslate(Component target, float x, float y)
    {
        var rectTransform = ResolveRectTransform(target);
        var state = BoomHudMotionRectState.Capture(rectTransform);
        rectTransform.anchoredPosition = new Vector2(state.AnchoredPosition.x + x, state.AnchoredPosition.y - y);
    }

    private static void ApplyScale(Component target, float x, float y)
    {
        var state = BoomHudMotionRectState.Capture(target);
        target.transform.localScale = new Vector3(state.LocalScale.x * x, state.LocalScale.y * y, state.LocalScale.z);
    }

    private static void ApplyRotation(Component target, float degrees)
    {
        var state = BoomHudMotionRectState.Capture(target);
        var euler = state.LocalEulerAngles;
        target.transform.localEulerAngles = new Vector3(euler.x, euler.y, euler.z + degrees);
    }

    private static void ApplyColor(Component target, string value)
    {
        ...
        var attachedGraphic = target.GetComponent<Graphic>();
        if (attachedGraphic != null)
        {
            attachedGraphic.color = color;
        }
    }

    private static void ApplyText(Component target, string value)
    {
        switch (target)
        {
            case Text text:
            ...
            case InputField inputField:
                ...
                {
                    inputField.textComponent.text = value;
                }
                return;
            case Button button:
                SetNestedLabelText(button.gameObject, value);
                return;
            case Toggle toggle:
                SetNestedLabelText(toggle.gameObject, value);
                return;
        }
    }

    private static void SetNestedLabelText(GameObject gameObject, string value)
    {
    ...
    private static RectTransform ResolveRectTransform(Component target)
    {
        return target as RectTransform
            ?? target.GetComponent<RectTransform>()
            ?? throw new InvalidOperationException($"Motion target '{target.name}' is missing a RectTransform.");
    }

...
public static class Program
{
    public static async Task<int> Main(string[] args)
    {
        try
        {
            ...
            var baselineCompareCommand = new Command("compare", "Compare current snapshots against baseline (hash-level)");
            var compareCurrentOption = new Option<DirectoryInfo?>("--current", "Current snapshots directory (with snapshots.manifest.json)");
            compareCurrentOption.AddAlias("-c");
            var compareBaselineOption = new Option<DirectoryInfo?>("--baseline", "Baseline snapshots directory (with snapshots.manifest.json)");
            compareBaselineOption.AddAlias("-b");
            var compareOutOption = new Option<FileInfo?>("--out", "Output report file (default: baseline-report.json in current directory)");
            compareOutOption.AddAlias("-o");
            var compareSummaryOption = new Option<bool>("--summary", () => false, "Print summary to stdout (for CI)");
            var compareFailOnChangedOption = new Option<bool>("--fail-on-changed", () => false, "Exit with code 1 if any actionable frames changed (legacy, use --fail-on)");
            var compareFailOnOption = new Option<string?>("--fail-on", "Fail condition: 'any' or 'percent:X' (e.g., 'percent:0.5' fails if any frame exceeds 0.5% changed)");
            var compareToleranceOption = new Option<int>("--tolerance", () => 0, "Per-channel delta tolerance (0-255). Pixels within tolerance are considered unchanged.");
            var compareMinChangedOption = new Option<double>("--min-changed-percent", () => 0.0, "Minimum changed percent to report as changed (noise filter)");
            var compareProtectedOption = new Option<string[]>("--protected", "Protected frame names (stricter: always fail if changed, regardless of threshold)")
            {
                Arity = ArgumentArity.ZeroOrMore,
                AllowMultipleArgumentsPerToken = true
            };
            var compareGhActionsOption = new Option<bool>("--gh-actions", () => false, "Output GitHub Actions step summary format");
            var compareVerboseOption = new Option<bool>("--verbose", () => false, "Enable verbose output");

            baselineCompareCommand.AddOption(compareCurrentOption);
            baselineCompareCommand.AddOption(compareBaselineOption);
            baselineCompareCommand.AddOption(compareOutOption);
            baselineCompareCommand.AddOption(compareSummaryOption);
            baselineCompareCommand.AddOption(compareFailOnChangedOption);
            baselineCompareCommand.AddOption(compareFailOnOption);
            baselineCompareCommand.AddOption(compareToleranceOption);
            baselineCompareCommand.AddOption(compareMinChangedOption);
            baselineCompareCommand.AddOption(compareProtectedOption);
            baselineCompareCommand.AddOption(compareGhActionsOption);
            baselineCompareCommand.AddOption(compareVerboseOption);

            baselineCompareCommand.SetHandler((InvocationContext context) =>
            {
                var currentDir = context.ParseResult.GetValueForOption(compareCurrentOption);
                var baselineDir = context.ParseResult.GetValueForOption(compareBaselineOption);
                var outFile = context.ParseResult.GetValueForOption(compareOutOption);
                var summary = context.ParseResult.GetValueForOption(compareSummaryOption);
                var failOnChanged = context.ParseResult.GetValueForOption(compareFailOnChangedOption);
                var failOn = context.ParseResult.GetValueForOption(compareFailOnOption);
                var tolerance = context.ParseResult.GetValueForOption(compareToleranceOption);
                var minChangedPercent = context.ParseResult.GetValueForOption(compareMinChangedOption);
                var protectedFrames = context.ParseResult.GetValueForOption(compareProtectedOption) ?? [];
                var ghActions = context.ParseResult.GetValueForOption(compareGhActionsOption);
                var verbose = context.ParseResult.GetValueForOption(compareVerboseOption);

                context.ExitCode = HandleBaselineCompare(
                    currentDir, baselineDir, outFile, summary, 
                    failOnChanged, failOn, tolerance, minChangedPercent, protectedFrames,
                    ghActions, verbose);
            });
        ...
        bool verbose)
    {
        try
        {
            ...
            var failThresholdPercent = ParseFailOnThreshold(failOn, out var failOnAny);
            if (failOnChanged) failOnAny = true; // Legacy flag
            ...
            if (!Directory.Exists(baselinePath))
            {
                ...
                WriteReport(noBaselineReport, outFile, currentPath, ghActions, printSummary, verbose);

                return failOnAny && noBaselineReport.Summary.Changed > 0 ? 1 : 0;
            }
            ...
            if (verbose)
            {
                ...
                if (minChangedPercent > 0) Console.WriteLine($"  Min changed %: {minChangedPercent}");
                if (protectedFrames.Length > 0) Console.WriteLine($"  Protected frames: {string.Join(", ", protectedFrames)}");
            }
            ...
            var report = CompareManifestsWithMetrics(
                ...
                tolerance, minChangedPercent, protectedFrames);

            // Write report
            WriteReport(report, outFile, currentPath, ghActions, printSummary, verbose);

            // Determine exit code based on fail conditions
            return DetermineFailExitCode(report, failOnAny, failThresholdPercent, protectedFrames);
        }
        catch (Exception ex)
        {
            Console.Error.WriteLine($"Error: {ex.Message}");
    ...
    private static double? ParseFailOnThreshold(string? failOn, out bool failOnAny)
    {
        failOnAny = false;
        if (string.IsNullOrEmpty(failOn)) return null;
        ...
        if (failOn.StartsWith("percent:", StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (double.TryParse(valueStr, System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out var threshold))
            {
                return threshold;
            }
        }
    ...
    private static int DetermineFailExitCode(BaselineReport report, bool failOnAny, double? failThresholdPercent, string[] protectedFrames)
    {
        ...
        // Check threshold
        if (failThresholdPercent.HasValue)
        {
            var exceeding = actionableChanged
                .Where(f => f.DiffMetrics != null && f.DiffMetrics.ChangedPercent > failThresholdPercent.Value)
                .ToList();

            if (exceeding.Count > 0)
            {
                Console.Error.WriteLine($"Error: {exceeding.Count} frame(s) exceed {failThresholdPercent.Value}% threshold");
                return 1;
            }
        }
    ...
    private static BaselineReport CompareManifestsWithMetrics(
        SnapshotOutputManifest current,
        SnapshotOutputManifest baseline,
        string currentPath,
        string baselinePath,
    ...
    private static DiffMetrics ComputeDiffMetrics(string baselinePath, string currentPath, int tolerance)
    {

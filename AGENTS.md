...
The ref repo ships **current Terminal.Gui v2** (net10.0, namespace-split, `IApplication`-based). Some consumers — including `fantasim-app-godot/project/hosts/complete-app/` — are stuck on **Terminal.Gui 2.0.0** (older flat-namespace alpha) because `Godot.NET.Sdk/4.6.3` only accepts net8.0 and 2.0.1+ requires net10.0.
